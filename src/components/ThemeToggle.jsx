...
export default function ThemeToggle({ theme, onToggle }) {
  ...
  return (
    <button
      ...
      className={`relative flex h-8 w-14 shrink-0 items-center rounded-full border border-[var(--border)] transition-colors duration-300 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)] ${
        isDark ? "bg-[var(--surface-2)]" : "bg-[var(--accent-soft)]"
      }`}
    >
      <Sun
      ...
      <span
        ...
        className={`grid h-6 w-6 place-items-center rounded-full bg-[var(--accent)] text-white shadow transition-transform duration-300 ease-out will-change-transform ${
          isDark ? "translate-x-7" : "translate-x-1"
        }`}
      >
        {isDark ? <Moon size={13} /> : <Sun size={13} />}
