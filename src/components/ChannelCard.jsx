...
//     <button
//       onClick={onSelect}
//       className={`group relative flex flex-col items-center gap-2 rounded-xl border p-3 text-center transition-colors duration-200 ${
//         active
...
//       <span className="grid h-12 w-12 place-items-center overflow-hidden rounded-lg bg-[var(--surface-2)]">
//         {status === "loading" && <span className="skeleton absolute inset-0 rounded-lg" />}
...
export default function ChannelCard({ channel, active, onSelect }) {
  ...
  return (
    <button
      ...
      className={`group relative flex flex-col items-center gap-2 rounded-xl border p-3 text-center transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)] ${
        active
          ? "border-[var(--accent)] bg-[var(--accent-soft)]"
          : "border-[var(--border)] bg-[var(--surface)] hover:border-[var(--accent)]/40"
      }`}
